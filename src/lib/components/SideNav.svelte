...
>
	...
	<div class="grid gap-2 content-start">
		...
		>
			<Library class="size-4 mr-2" aria-hidden="true" /> {t('nav.library')}
		</button>
		<button
			onclick={() => goto('/player')}
			class="flex items-center justify-start gap-2 min-h-[42px] px-3 text-soft text-left hover:bg-brand/10 hover:text-fg focus-visible:bg-brand/10 focus-visible:text-fg"
		...
		>
			<Play class="size-4 mr-2" aria-hidden="true" /> {t('nav.player')}
		</button>
		<button
		...
		>
			<Settings class="size-4 mr-2" aria-hidden="true" /> {t('nav.settings')}
		</button>
	</div>
	<section class="min-w-0 border border-outline rounded-3xl p-4 bg-surface-2/56">
		<p class="m-0 mb-2 text-soft font-mono text-xs tracking-widest uppercase">{t('common.output')}</p>

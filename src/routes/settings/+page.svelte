<script lang="ts">
	...
	let account: JellyfinAccount | null = $state(null);
	let audioOutputs: AudioOutputDevice[] = $state([]);
	let serviceCapabilities: ProviderCapability[] = $state([]);
	let providerAccounts: ProviderAccount[] = $state([]);
	let providerLoginStates: ProviderLoginState[] = $state([]);
	let providerStatuses: ProviderStatus[] = $state([]);
	let lastFmScrobbleStatus: LastFmScrobbleStatus | null = $state(null);
	let baseUrl = $state('');
	let userName = $state('');
	let password = $state('');
	let selectedAudioOutput = $state('default');
	let replayGainMode = $state('off');
	let crossfadeEnabled = $state(false);
	let crossfadeDuration = $state(3000);
	let monoDownmix = $state(false);
	let preampGainDb = $state(0);
	let playbackSpeed = $state(1.0);
	let playbackSpeedValue = $state('1.0');
	let notificationsEnabled = $state(true);
	let selectedProviderId = $state('spotify');
	let providerDisplayName = $state('');
	let providerClientId = $state('');
	let providerClientSecret = $state('');
	let providerApiKey = $state('');
	let providerApiSecret = $state('');
	...
	let tidalRedirectUri = $state('http://127.0.0.1:9090/callback');
	let tidalScope = $state('search.read playlists.read user.read');
	let tidalAuthorizationCode = $state('');
	let tidalAuthorizationState = $state('');
	let tidalAuthorizationUrl = $state('');
	let youtubeRedirectUri = $state('http://127.0.0.1:9090/callback');
	let youtubeScope = $state('https://www.googleapis.com/auth/youtube.readonly');
	let youtubeAuthorizationCode = $state('');
	let youtubeAuthorizationState = $state('');
	let youtubeAuthorizationUrl = $state('');
	let lastFmAuthorizationUrl = $state('');

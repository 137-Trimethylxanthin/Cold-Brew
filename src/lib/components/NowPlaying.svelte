...
<div class="grid gap-[18px]">
	...
	<div class="grid gap-[6px]">
		...
		<p class="m-0 text-soft font-mono text-[0.82rem]">{detail()}</p>
		{#if song.artist}
			<span class="text-soft text-[0.9rem]">{song.artist}</span>
		{/if}
		{#if song.album}
			<span class="text-soft text-[0.9rem]">{song.album}</span>
		{/if}
	</div>
</div>
